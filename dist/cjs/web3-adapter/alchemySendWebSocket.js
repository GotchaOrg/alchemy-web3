...
function makeWebSocketSender(ws) {
  ...
  ws.addEventListener("down", function () {
    tslib_1
      .__spreadArray([], tslib_1.__read(contextsById))
      .forEach(function (_a) {
        ...
        if (isWrite(request)) {
          ...
          resolve({
            ...
            status: 0,
            message:
              "WebSocket closed before receiving a response for write request with id: " +
              id +
              ".",
          });
        }
  ...
  ws.addEventListener("reopen", function () {
    ...
    try {
      for (
        var _b = tslib_1.__values(contextsById.values()), _c = _b.next();
        !_c.done;
        _c = _b.next()
      ) {
...
function getIdFromRequest(request) {
  ...
  return getCanonicalIdFromList(
    request.map(function (p) {
      return p.id;
    }),
  );
}
...
function getCanonicalIdFromList(ids) {
  ...
  if (numberIds.length > 0) {
    return Math.min.apply(
      Math,
      tslib_1.__spreadArray([], tslib_1.__read(numberIds)),
    );
  }
