...
function makeAlchemyContext(url, config) {
  ...
  if (/^https?:\/\//.test(url)) {
    ...
    var _a = sendJsonRpcPayload_1.makeJsonRpcPayloadSender(
        alchemySendJsonrRpc,
        config,
      ),
      sendJsonRpcPayload = _a.sendJsonRpcPayload,
      setWriteProvider = _a.setWriteProvider;
    var jsonRpcSenders = jsonRpc_1.makeJsonRpcSenders(
      sendJsonRpcPayload,
      makeJsonRpcPayload,
    );
    ...
    var protocol = isAlchemyUrl(url)
      ? "alchemy-web3-" + version_1.VERSION
      : undefined;
    ...
    var _b = sendJsonRpcPayload_1.makeJsonRpcPayloadSender(alchemySend, config),
    ...
    var provider = new webSocketProvider_1.AlchemyWebSocketProvider(
      ws,
      sendJsonRpcPayload,
      jsonRpcSenders,
    );
    ...
    throw new Error(
      "Alchemy URL protocol must be one of http, https, ws, or wss. Recieved: " +
        url,
    );
  }
}
...
function getWebSocketConstructor() {
  return isNodeEnvironment()
    ? function (url, protocols) {
        return new websocket_1.w3cwebsocket(
          url,
          protocols,
          undefined,
          undefined,
